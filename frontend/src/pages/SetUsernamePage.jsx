...
export default function SetUsernamePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await changeUsername(userId, username.trim());
      // After setting username, navigate to chat
      navigate('/chat');
    } catch (e) {
      ...
      <div className="relative z-10 w-full max-w-sm px-4">
        <div className="bg-surface border border-border rounded-3xl p-8 shadow-2xl animate-slide-up">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-tx-1 text-2xl font-bold tracking-tight">Set Username</h1>
            <p className="text-tx-2 text-sm mt-1">Choose a unique username</p>
          </div>
