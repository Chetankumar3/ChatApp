...
import { Rate, Gauge, Counter, Trend } from 'k6/metrics';
import { randomString } from 'https://jslib.k6.io/k6-utils/1.2.0/index.js';
import encoding from 'k6/encoding';
import exec from 'k6/execution';

// ─── Environment ─────────────────────────────────────────────────────────────
const IP = "35.253.93.168";
...
const registrationTime = new Trend('registration_duration_ms');
...
export default function () {
  ...
  if (!isSetupComplete) {
    ...
    registrationTime.add(Date.now() - registrationStart);
  ...
  // 2. WebSocket Session
  const MAX_MESSAGES     = 50 + Math.floor(Math.random() * 150); // 10–20 msgs
